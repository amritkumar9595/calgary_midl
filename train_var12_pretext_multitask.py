...
from data.data_loader2 import SliceData , DataTransform_slicemiss, DataTransform_multitask
...
def create_datasets(args,data_path):
    ...
    train_data = SliceData(
        ...
        transform=DataTransform_multitask(),
...
def train_epoch(args, epoch,model, model_cla, data_loader,optimizer,criterion,writer):
    ...
    for iter, data in enumerate(tqdm(data_loader)):
        ...
        ksp_us, img_us, img_us_rss, img_us_np, img_gt_np, label, sens, mask, maxi, fname,_,_ = data
        ...
        print("out=",out.shape,"img_us_np=",img_us_np.shape)
        ...
        # print("out",out.shape)
        cla_in = out_stack[-2]
        print("cla_in=",cla_in.shape)
        print("sens=",sens.shape)
        cla_in = T.rss(cla_in,sens).float()
        pred = model_cla(out)
        
        # print("out",out.shape)
        # loss_cmplx = F.mse_loss(out,img_gt_np.cuda())

        loss_rot = criterion(pred, label)
        ...
        writer.add_scalar('Train_Loss_cmplx', loss_cmplx.item(), global_step + iter)
        writer.add_scalar('Train_Loss_rot', loss_rot.item(), global_step + iter)
...
def evaluate(args, epoch,  model ,model_cla,  data_loader , criterion, writer):
    ...
    with torch.no_grad():
        ...
        # print("out",out.shape)
            cla_in = out_stack[-2]
            cla_in = T.rss(cla_in.squeeze(0),sens).float()
            pred = model_cla(cla_in)
        ...
        #     loss_cmplx_ssim =  ssim_loss(out, img_gt_np,torch.tensor(img_gt_np.max().item()).unsqueeze(0).cuda())
            ...
            losses_cla.append(loss_rot.item())
            losses.append(loss.item())

        # wandb.log({"Dev_Loss_rotnet": loss.item() })

    writer.add_scalar('Dev_Loss_cmplx',np.mean(losses_cmplx), epoch)
    writer.add_scalar('Dev_Loss_cla',np.mean(losses_cla), epoch)
    writer.add_scalar('Dev_Loss_comb',np.mean(losses), epoch)
...
def main(args):
    ...
    print("PRETEXT Training of VarNet from  with 12-channels data, using",args.loss, " MULTI-PRETEXT LEARNING")

    train_loader, dev_loader , display_loader = create_data_loaders(args,args.data_path)  
    print("dataloaders for 12 channels MULTI-TASK PRETEXT LEARNING data readdy")  
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        # train_loss_cmplx, train_time = train_epoch(args, epoch,  model, model_cla,  train_loader, optimizer, writer)
        train_loss_cmplx, train_time = train_epoch(args, epoch,  model ,model_cla , train_loader, optimizer, criterion, writer)

        # dev_loss_cmplx , dev_time = evaluate1(args, epoch, model, model_cla,  dev_loader, writer)
        dev_loss_cmplx2 , dev_time2 = evaluate2(args, epoch, model , model_cla,  dev_loader, criterion, writer)
...
def create_arg_parser():
    ...
    parser.add_argument('--lr', type=float, default=0.001, help='Learning rate for slicemiss')
