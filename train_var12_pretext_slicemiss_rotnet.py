"""
Pretext train VarNet with slicemiss + rotnet , but in a alternative way with 12 channels data
...
from data.data_loader2 import SliceData , DataTransform_slicemiss, DataTransform_rotnet
from models.models import  network, SSIM ,SensitivityModel, architecture_unet, classifier
...
def create_datasets1(args,data_path):

    train_data = SliceData(
        root=str(data_path) + '/Train',
        transform=DataTransform_slicemiss(),
    ...
    dev_data = SliceData(
        ...
        transform=DataTransform_slicemiss(),
        no_of_vol=1,             #args.sample_rate,
        acceleration=args.acceleration
    )
    return dev_data, train_data

def create_datasets2(args,data_path):
...
def create_data_loaders1(args,data_path):
    dev_data, train_data = create_datasets1(args,data_path)
...
def create_data_loaders2(args,data_path):
    dev_data, train_data = create_datasets2(args,data_path)
    display_data = [dev_data[i] for i in range(0, len(dev_data) // 16)]
...
ssim_loss = SSIM().cuda()
def train_epoch1(args, epoch,model, data_loader,optimizer, writer):
    ...
    for iter, data in enumerate(tqdm(data_loader)):
        ...
        ksp_us,img_us,img_us_rss,img_us_np,img_gt_np,sens,mask,maxi,fname,_,_ = data
        ...
        # print("fname",fname)
        # print("img_us,ksp_us,img_gt_np",img_us.shape,ksp_us.shape,img_gt_np.shape)
...
def train_epoch2(args, epoch,model, model_cla, data_loader,optimizer,criterion,writer):
    ...
    for iter, data in enumerate(tqdm(data_loader)):
        ...
        writer.add_scalar('Train_Loss2', loss.item(), global_step + iter)
        # writer.add_scalar('TrainLoss_cmplx_mse', loss_cmplx_mse.item(), global_step + iter)
        ...
        if iter % args.report_interval == 0:
            ...
            f'Epoch = [{epoch:3d}/{args.num_epochs:3d}] '
            f'Iter = [{iter:4d}/{len(data_loader):4d}] '
            f'Loss2 = {loss.item():.4g} Avg Loss2 = {avg_loss:.4g} '
            f'Time2 = {time.perf_counter() - start_iter:.4f}s',
        )
        start_iter = time.perf_counter()
...
def evaluate1(args, epoch,  model ,   data_loader, writer):
    ...
    with torch.no_grad():
        ...
        for iter, data in enumerate(tqdm(data_loader)):
            ...
            ksp_us,img_us,img_us_rss,img_us_np,img_gt_np,_,mask,maxi,fname,_,_ = data
...
def evaluate2(args, epoch,  model ,model_cla,  data_loader , criterion, writer):
    ...
    writer.add_scalar('Dev_Loss2',np.mean(losses), epoch)
        # writer.add_scalar('Dev_Loss_cmplx_mse',loss_cmplx_mse, epoch)
...
def visualize(args, epoch,  model, data_loader, writer):
    ...
    with torch.no_grad():
        ...
        for iter, data in enumerate(data_loader):
            ...
            ksp_us ,img_us,img_us_rss,img_us_np,img_gt_np , sens,mask,_,_,r_int,img_gt = data
            ...
            img_gt_cmplx_abs = (torch.sqrt(img_gt[0,:,:,:,0]**2 + img_gt[0,:,:,:,1]**2)).unsqueeze(1).to(args.device)
            ...
            save_image(img_gt_cmplx_abs,'GT-image')
            save_image(sens_cmplx_abs,'sens')
            save_image(sens_cmplx_abs_ori,'sens_map_from_enlive')
            
            # r_int = int(r_int)
            # print("r_int=",type(int(r_int[0])))
            end_cmplx = out_stack[-1]
            print("end_cmplx",end_cmplx.shape)
            # x = out_stack[r_int.item(),:,:,:]
            out_ch = T.rss_ch(end_cmplx,sens).float()
            print("out_ch=",out_ch.shape)
            recons_ch = out_ch[0,r_int.item(),:,:]

            # print("img_gt_cmplx_abs",img_gt_cmplx_abs[r_int.item()].shape)


            save_image(recons_ch,'reconstruction missing channel')
            save_image(img_gt_cmplx_abs[r_int.item()].unsqueeze(0),'GT for missing channel')
            # save_image(img_us_cmplx_abs[r_int.item()].unsqueeze(0),'US for missing channel')
            ...
            print("out_statck",len(out_stack))
...
def visualize2(args, epoch,  model, model_cla, data_loader,writer):
    def save_image(image,tag):
        image -= image.min()
        image /= image.max()
        # image = image[0,0,:,:].cpu().numpy()
    ...
    with torch.no_grad():
        ...
        for iter, data in enumerate(data_loader):
            ...
            # print("out",out.shape)
            pred = model_cla(out)
            print("pred2=",torch.argmax(pred),"true_label2=",label)
            ...
            # save_image(error_cmplx,'Error')
            out = save_image(out, 'Recons2')
            # print("img_us",img_us_np.shape)
            # wandb.log({"img": [wandb.Image(norm_image(img_us_np.unsqueeze(0)), caption="US-Image"), wandb.Image(norm_image(out), caption="Reconstruction"),
            # wandb.Image(norm_image(img_gt_np.unsqueeze(0)), caption="GT-Image"),wandb.Image(norm_image(error_cmplx), caption="Error")]})
            # save_image(img_gt_np,'Target')
            
            save_image(img_gt_np,'GT-image_rss2')
            save_image(img_us_np,'US-image_rss2')
            save_image(img_us_cmplx_abs,'US-image2')
            save_image(sens_cmplx_abs,'sens2')
            save_image(sens_cmplx_abs_ori,'sens_map_from_enlive2')
            ...
            out_cmplx_abs = (torch.sqrt(out_cmplx[:,:,:,0]**2 + out_cmplx[:,:,:,1]**2)).unsqueeze(1).to(args.device)
            out_cmplx_abs  = T.pad(out_cmplx_abs[0,0,:,:],[256,256]).unsqueeze(0).unsqueeze(1).to(args.device)  
            save_image(out_cmplx_abs, 'Recons02')
            
            out_cmplx=out_stack[1]
            out_cmplx_abs = (torch.sqrt(out_cmplx[:,:,:,0]**2 + out_cmplx[:,:,:,1]**2)).unsqueeze(1).to(args.device)
            print('out_cmplx_abs2',out_cmplx_abs.shape)
            out_cmplx_abs  = T.pad(out_cmplx_abs[0,0,:,:],[256,256]).unsqueeze(0).unsqueeze(1).to(args.device)  
            save_image(out_cmplx_abs, 'Recons12')
            
            out_cmplx=out_stack[2]
            out_cmplx_abs = (torch.sqrt(out_cmplx[:,:,:,0]**2 + out_cmplx[:,:,:,1]**2)).unsqueeze(1).to(args.device)
            out_cmplx_abs  = T.pad(out_cmplx_abs[0,0,:,:],[256,256]).unsqueeze(0).unsqueeze(1).to(args.device)  
            save_image(out_cmplx_abs, 'Recons22')
            
            out_cmplx=out_stack[-2]
            out_cmplx_abs = (torch.sqrt(out_cmplx[:,:,:,0]**2 + out_cmplx[:,:,:,1]**2)).unsqueeze(1).to(args.device)
            out_cmplx_abs  = T.pad(out_cmplx_abs[0,0,:,:],[256,256]).unsqueeze(0).unsqueeze(1).to(args.device)  
            save_image(out_cmplx_abs, 'Recons32')
            
            out_cmplx=out_stack[-1]
            out_cmplx_abs = (torch.sqrt(out_cmplx[:,:,:,0]**2 + out_cmplx[:,:,:,1]**2)).unsqueeze(1).to(args.device)
            out_cmplx_abs  = T.pad(out_cmplx_abs[0,0,:,:],[256,256]).unsqueeze(0).unsqueeze(1).to(args.device)  
            save_image(out_cmplx_abs, 'Recons42')
...
def save_model(args, exp_dir, epoch, model, model_cla, optimizer1,optimizer2,best_dev_loss_cmplx,is_new_best_cmplx):
...
def load_model(checkpoint_file):
    ...
    model_cla.load_state_dict(checkpoint['model_cla'])

    optimizer1 = build_optim(model.parameters(),args.lr1,args.weight_decay)
    optimizer2 = build_optim(list(model.parameters()) + list(model_cla.parameters()),args.lr2,args.weight_decay)
    ...
    return checkpoint,  model , model_cla , optimizer1 , optimizer2
...
def main(args):
    ...
    print("PRETEXT Training of VarNet from  with 12-channels data, using",args.loss,"  taking US image as GT and ROTNET")

    train_loader, dev_loader , display_loader = create_data_loaders1(args,args.data_path)  
    print("dataloaders for 12 channels slicemiss data readdy")  

    train_loader2, dev_loader2 , display_loader2 = create_data_loaders2(args,args.data_path)  
    print("dataloaders for 12 channels rotnet data readdy")

    scheduler_vs1 = torch.optim.lr_scheduler.StepLR(optimizer1, args.lr_step_size, args.lr_gamma)
    scheduler_vs2 = torch.optim.lr_scheduler.StepLR(optimizer2, args.lr_step_size, args.lr_gamma)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        train_loss_cmplx, train_time = train_epoch1(args, epoch,  model ,  train_loader, optimizer1, writer)
        # train_loss_cmplx2, train_time2 = train_epoch2(args, epoch,  model ,model_cla , train_loader2, optimizer2, criterion, writer)

        dev_loss_cmplx , dev_time = evaluate1(args, epoch, model ,  dev_loader, writer)
        # dev_loss_cmplx2 , dev_time2 = evaluate2(args, epoch, model , model_cla,  dev_loader2, criterion, writer)

        visualize(args, epoch, model ,  display_loader, writer)
        # visualize2(args, epoch, model , model_cla,  display_loader2, writer)

        scheduler_vs1.step(epoch)
        scheduler_vs2.step(epoch)
        
        is_new_best_cmplx = dev_loss_cmplx < best_dev_loss_cmplx

        best_dev_loss_cmplx = min(best_dev_loss_cmplx, dev_loss_cmplx)
                
        save_model(args, args.exp_dir, epoch, model, model_cla, optimizer1, optimizer2, best_dev_loss_cmplx ,is_new_best_cmplx)
        logging.info(
        ...
        # logging.info(
        #     f'Epoch = [{epoch:4d}/{args.num_epochs:4d}]  TrainLoss_cmplx2 = {train_loss_cmplx2:.4g}  '
        #     f'DevLoss_cmplx2 = {dev_loss_cmplx2:.4g} TrainTime = {train_time2:.4f}s DevTime = {dev_time2:.4f}s',
        # )
...
def create_arg_parser():
    ...
    parser.add_argument('--lr1', type=float, default=0.001, help='Learning rate for slicemiss')
    parser.add_argument('--lr2', type=float, default=0.000001, help='Learning rate for classification')
